...
class TransformerBlock:
  def __init__(self, dim:int, hidden_dim:int, n_heads:int, n_kv_heads:int, norm_eps:float, head_dim:int, rope_theta:float,
               max_context:int=0, qk_norm:int=0, num_experts:int=0, num_experts_per_tok:int=0, causal:bool=True):
  ...
  def _attention(self, x:Tensor, start_pos:int|UOp) -> Tensor:
    ...
    use_causal = self.causal and isinstance(start_pos, int) and start_pos == 0
    use_cache = self.causal and not (Tensor.training and use_causal)
    if use_cache:
...
=======
    mask = None
    if not use_causal and T > 1:
      mask = Tensor.full((1, 1, T, start_pos+T), float("-inf"), dtype=x.dtype, device=x.device).triu(start_pos+1)
    attn = q.scaled_dot_product_attention(k, v, attn_mask=mask, is_causal=use_causal, enable_gqa=True)     # (B,H,T,Hd)
...
class DeepSeek2Block:
  def __init__(self, dim:int, ffn_hidden_dim:int, n_heads:int, key_dim:int, value_dim:int, norm_eps:float, rope_theta:float, rope_dim:int,
               q_lora_rank:int, kv_lora_rank:int, max_context:int=0, num_experts:int=0, num_experts_per_tok:int=0, expert_shared_count:int=0,
               causal:bool=True):
    self.n_heads = n_heads
    ...
    self.attn_q_b = nn.Linear(q_lora_rank, n_heads * key_dim, bias=False)
    self.attn_kv_a_mqa = nn.Linear(dim, kv_lora_rank + rope_dim, bias=False)
    self.attn_kv_a_norm = nn.RMSNorm(kv_lora_rank, norm_eps)
    self.attn_k_b = Weight(n_heads, kv_lora_rank, key_dim - rope_dim)
    self.attn_v_b = Weight(n_heads, value_dim, kv_lora_rank)
    self.attn_output = nn.Linear(n_heads * value_dim, dim, bias=False)
    ...
    if num_experts > 0:
      ...
      self.ffn_gate_exps = ExpertWeights(num_experts, dim, ffn_hidden_dim)
      self.ffn_up_exps = ExpertWeights(num_experts, dim, ffn_hidden_dim)
      self.ffn_down_exps = ExpertWeights(num_experts, ffn_hidden_dim, dim)
      if expert_shared_count > 0:
        self.ffn_gate_shexp = nn.Linear(dim, ffn_hidden_dim, bias=False)
        self.ffn_up_shexp = nn.Linear(dim, ffn_hidden_dim, bias=False)
        self.ffn_down_shexp = nn.Linear(ffn_hidden_dim, dim, bias=False)
    else:
      self.ffn_gate = nn.Linear(dim, ffn_hidden_dim, bias=False)
      self.ffn_up = nn.Linear(dim, ffn_hidden_dim, bias=False)
      self.ffn_down = nn.Linear(ffn_hidden_dim, dim, bias=False)

  def _attention(self, x:Tensor, start_pos:int|UOp) -> Tensor:
    x_norm = self.attn_norm(x)
    q = self.attn_q_b(self.attn_q_a_norm(self.attn_q_a(x_norm)))
    kv = self.attn_kv_a_mqa(x_norm)
    kv_lora, k_pe = kv[:, :, :self.kv_lora_rank], kv[:, :, self.kv_lora_rank:]
    kv_lora = self.attn_kv_a_norm(kv_lora)

    B, T, _ = x.shape
    q = q.reshape(B, T, self.n_heads, self.key_dim).transpose(1, 2)
    q_nope, q_pe = q[:, :, :, :-self.rope_dim], q[:, :, :, -self.rope_dim:]
    k_nope = (kv_lora.unsqueeze(1) @ self.attn_k_b.weight).transpose(1, 2)
    v = (kv_lora.unsqueeze(1) @ self.attn_v_b.weight.transpose(-1, -2)).transpose(1, 2)
    k_pe = k_pe.unsqueeze(1).expand((B, self.n_heads, T, self.rope_dim))
    ...
    if use_cache:
      if self.key_dim == self.value_dim:
        if not hasattr(self, "cache_kv"):
          self.cache_kv = Tensor.zeros(2, B, self.n_heads, self.max_context, self.key_dim, dtype=k.dtype, device=k.device).contiguous().realize()
        self.cache_kv[:, :, :, start_pos:start_pos+T, :].assign(Tensor.stack(k, v)).realize()
        k = self.cache_kv[0, :, :, 0:start_pos+T, :]
        v = self.cache_kv[1, :, :, 0:start_pos+T, :]
      else:
        if not hasattr(self, "cache_k"):
          self.cache_k = Tensor.zeros(B, self.n_heads, self.max_context, self.key_dim, dtype=k.dtype, device=k.device).contiguous().realize()
          self.cache_v = Tensor.zeros(B, self.n_heads, self.max_context, self.value_dim, dtype=v.dtype, device=v.device).contiguous().realize()
        self.cache_k[:, :, start_pos:start_pos+T, :].assign(k).realize()
        self.cache_v[:, :, start_pos:start_pos+T, :].assign(v).realize()
    ...
    if not use_causal and T > 1:
      mask = Tensor.full((1, 1, T, start_pos+T), float("-inf"), dtype=x.dtype, device=x.device).triu(start_pos+1)
    attn = q.scaled_dot_product_attention(k, v, attn_mask=mask, is_causal=use_causal, enable_gqa=False)
  ...
  def _feed_forward(self, h: Tensor) -> Tensor:
    ...
    if hasattr(self, 'ffn_gate_exps'):
      ...
      logits = self.ffn_gate_inp(h_norm) + self.exp_probs_b.bias
      ...
      routed = (x_down * probs.unsqueeze(-1)).sum(axis=2)
...
class DeepSeek2Transformer:
  def __init__(self, *, num_blocks:int, dim:int, ffn_hidden_dim:int, expert_ffn_hidden_dim:int, n_heads:int, key_dim:int, value_dim:int,
               norm_eps:float, vocab_size:int, rope_theta:float, rope_dim:int, q_lora_rank:int, kv_lora_rank:int, max_context:int,
               num_experts:int, num_experts_per_tok:int, expert_shared_count:int, leading_dense_blocks:int=0, causal:bool=True):
    self.blk = [
      DeepSeek2Block(dim, ffn_hidden_dim if i < leading_dense_blocks else expert_ffn_hidden_dim, n_heads, key_dim, value_dim, norm_eps, rope_theta,
                     rope_dim, q_lora_rank, kv_lora_rank, max_context=max_context, num_experts=0 if i < leading_dense_blocks else num_experts,
                     num_experts_per_tok=num_experts_per_tok, expert_shared_count=expert_shared_count, causal=causal)
...
class Transformer:
  def __init__(self, *, num_blocks, dim, hidden_dim, n_heads, n_kv_heads, norm_eps, vocab_size, head_dim:int, rope_theta:float,
    ...
    self.blk = [TransformerBlock(dim, hidden_dim, n_heads, n_kv_heads, norm_eps, head_dim, rope_theta, max_context, qk_norm,
                                 num_experts, num_experts_per_tok, causal) for _ in range(num_blocks)]
    self.token_embd  = nn.Embedding(vocab_size, dim)
    self.output_norm = nn.RMSNorm(dim, norm_eps)
    self.output = nn.Linear(dim, vocab_size, bias=False)
    self.max_context = max_context
  ...
  def from_gguf(gguf:Tensor, max_context:int|None=None, realize=True) -> tuple[Transformer, dict]:
    ...
    if arch == 'deepseek2':
      model = DeepSeek2Transformer(num_blocks=kv[f'{arch}.block_count'], dim=kv[f'{arch}.embedding_length'],
                                   ffn_hidden_dim=kv[f'{arch}.feed_forward_length'],
                                   expert_ffn_hidden_dim=kv[f'{arch}.expert_feed_forward_length'],
                                   n_heads=n_heads, key_dim=kv[f'{arch}.attention.key_length_mla'],
                                   value_dim=kv[f'{arch}.attention.value_length_mla'],
                                   norm_eps=kv[f'{arch}.attention.layer_norm_rms_epsilon'],
                                   vocab_size=len(kv['tokenizer.ggml.tokens']),
                                   rope_theta=kv[f'{arch}.rope.freq_base'], rope_dim=kv[f'{arch}.rope.dimension_count'],
                                   q_lora_rank=kv[f'{arch}.attention.q_lora_rank'], kv_lora_rank=kv[f'{arch}.attention.kv_lora_rank'],
                                   max_context=max_context, num_experts=kv[f'{arch}.expert_count'],
                                   num_experts_per_tok=kv[f'{arch}.expert_used_count'],
                                   expert_shared_count=kv.get(f'{arch}.expert_shared_count', 0),
...
models = {
  ...
  "glm4.7:flash": "https://huggingface.co/bartowski/zai-org_GLM-4.7-Flash-GGUF/resolve/main/zai-org_GLM-4.7-Flash-Q4_1.gguf"
